...
************************************************************************
 ...
 Script name       : HDMI_codec_test.sh
 Author            : lancey
...
AUDIOTESTLOG=/tmp/hdmi_audiotest_log.txt
...
#play with HW codec
SNDCARDID=$(aplay -l | grep -E "hdmi" | head -1 | awk '{print $2}' | cut -d ':' -f1)

#set volume_set , hdmi is not required
